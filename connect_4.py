...
class Joc:
    ...
    def get_diagonala1(self, linie, coloana):
        ...
        lista_return = []
        if linie + 4 < self.NR_LINII and coloana + 4 < self.NR_COLOANE:
	...
	# ia elemente de pe diagonala /
        lista_return = []
        if linie + 4 >= self.NR_LINII and coloana - 4 >= 0:
            for (i, j) in zip(range(linie, linie+4, -1), range(coloana, coloana-4, -1)):
                lista_return.append(self.matr[i][j])
        return lista_return
    ...
    def final(self):
        ...
        for linie in range(Joc.NR_LINII):
            for coloana in range(Joc.NR_COLOANE-3):
                counter = 0
                juc_curent = self.matr[linie][coloana]
                if juc_curent != '#':
                    for verif_col in range(coloana, coloana+4):
                        if self.matr[linie][verif_col] == juc_curent:
                            counter += 1
                    if counter == 4:
                        return juc_curent
        ...
        for coloana in range(Joc.NR_COLOANE):
            for linie in range(Joc.NR_LINII-3):
                ...
                juc_curent = self.matr[linie][coloana]
                if juc_curent != '#':
                    for verif_lin in range(linie, linie+4):
                        if self.matr[verif_lin][coloana] == juc_curent:
                            counter += 1
                    if counter == 4:
                        return juc_curent
        ...
        # diagonale \
        for linie in range(Joc.NR_LINII-3):
            for coloana in range(Joc.NR_COLOANE-2):
                diagonala = self.get_diagonala1(linie, coloana)
                diagonala = list(set(diagonala))
                if len(diagonala) == 1 and diagonala[0] != '#':
                    return diagonala[0]

        # diagonale /
        for linie in range(Joc.NR_LINII):
            for coloana in range(3, Joc.NR_COLOANE):
                diagonala = self.get_diagonala2(linie, coloana)
                diagonala = list(set(diagonala))
                if len(diagonala) == 1 and diagonala[0] != '#':
                    return diagonala[0]

        if nu_mai_e_gol(self.matr):
            return 'remiza'
        else:
            return False
    ...
    def nr_intervale_deschise(self, jucator):
        ...
        for linie in range(Joc.NR_LINII):
            for coloana in range(Joc.NR_COLOANE-3):
                lista_verificare = []
                for verif_col in range(coloana, coloana+4):
                    lista_verificare.append(self.matr[linie][verif_col])
                if juc_opus not in lista_verificare:
                    rez += 1
        ...
        for coloana in range(Joc.NR_COLOANE):
            for linie in range(Joc.NR_LINII-3):
                lista_verificare = []
                for verif_lin in range(linie, linie+4):
                    lista_verificare.append(self.matr[verif_lin][coloana])
                if juc_opus not in lista_verificare:
                    rez += 1
